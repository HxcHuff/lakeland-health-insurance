...
The browser accepts only registry-derived page roles, cluster names, and CTA keys. It does not trust `source_page_role` or `content_cluster` from a URL or hidden field. Campaign parameters are limited to validated `utm_source`, `utm_medium`, `utm_campaign`, and `utm_content`; contact-like values are rejected. Raw query strings, referrer URLs, `utm_term`, `gclid`, and `fbclid` are not copied into the Get Help attribution record.
